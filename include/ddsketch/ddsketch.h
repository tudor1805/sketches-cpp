...
class BinList {
 public:
    ...
    BinList& operator=(BinList<BinItem>&& bins) {
        data_ = std::move(bins.data_);
        return *this;
    }
...
class BaseStore : CRTP<ConcreteStore> {
 ...
 protected:
    ...
    BaseStore(BaseStore&& store) = default;

    BaseStore& operator=(const BaseStore& store) = default;
    BaseStore& operator=(BaseStore&& store) = default;
};
